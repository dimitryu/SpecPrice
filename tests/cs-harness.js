...
(async()=>{
  ...
  // connector links
  ok('form: every connector carries links', /class="cs-link/.test(html));
  ok('form: ...that search by its part number', /mouser\.com\/c\/\?q=TV06RW1135SF472A/.test(html));
  ok('form: ...and for the loose contact too', /55A0111-22-0/.test(html));
  ok('form: there is a way to fetch the real product pages',
  ...
  ok('wires: ...and gives the pieces to cut', /Cut pieces/.test(wh));
